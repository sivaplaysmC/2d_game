...
class Game:
    ...
    def __init__(self , rect_list:list) :
        ...
        for i in self.rect_list :
            ...
            if i.w > 500 and i.y < 500 :
                self.base = pygame.Rect(i.x , i.y  , i.w , i.h )
        ...
        for self.event in pygame.event.get() :
            ...
            if self.event.type == pygame.KEYDOWN :
                ...
                if self.event.key == pygame.K_a :
                    self.player2.actions["left"] = True
                if self.event.key == pygame.K_w :
                    self.player2.actions["up"] = True
                if self.event.key == pygame.K_d :
                    self.player2.actions["right"] = True
            ...
            if self.event.type == pygame.KEYUP :
                ...
                if self.event.key == pygame.K_a :
                    self.player2.actions["left"] = False
                if self.event.key == pygame.K_w :
                    self.player2.actions["up"] =    False
                if self.event.key == pygame.K_d :
                    self.player2.actions["right"] =  False
    ...
    def draw(self) :
        ...
        if self.game_state_stack.peek().name == "Main":
            ENVIRONMENT.fill('teal')
            ENVIRONMENT.blit(MAIN_MENU_IMAGE , (0,0))
        if self.is_dead :
            ENVIRONMENT.fill('white')
            ENVIRONMENT.blit(DEATH_MENU_IMAGE , (0,0))

        if self.victory :
            ENVIRONMENT.fill('darkslategray')
            ENVIRONMENT.blit(WIN_MENU_IMAGE, (0,0))
    ...
    def check_death(self) :
        if self.player1.rect.y > 661 :
            self.is_dead = 1
        if self.player2.rect.y > 661 :
            self.is_dead = 1

    def check_win(self) :
        print(self.player2.rect.topleft )

        if (self.player1.rect.x in range(84,183) ) :
            print('One')
            if  self.player1.rect.y == 180   :
                print('two')
                if  self.player2.rect.x in range(1164 , 1271) :
                    print('three')
                    if self.player2.rect.y == 186   :
                        self.victory  = 1
            pass
...
DEATH_MENU_IMAGE = pygame.image.load('Death.png').convert_alpha()
WIN_MENU_IMAGE = pygame.image.load('Victory.png').convert_alpha()



MAIN_MENU_IMAGE = pygame.image.load('Main.png').convert_alpha()
